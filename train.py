...
for epoch in range(max_epoch):
    ...
    for idx, (sequence,label) in enumerate(tqdm(train_dataloader)):
        ...
        if critic_step == 0:
            tqdm.write(f"[Epoch:{epoch}/{max_epoch}][Batch:{idx}/{len(train_dataloader)}][d_loss:{d_loss.item()}][g_loss:{g_loss.item()}]")
        else:
            tqdm.write(f"[Epoch:{epoch}/{max_epoch}][Batch:{idx}/{len(train_dataloader)}][d_loss:{d_loss.item()}]")
